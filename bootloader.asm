...
start:
    ...
    mov sp, 0x7C00 ; defini a pilha do mesmo local do bootloader
    ...
    jc error       ; se ouver erro pula pra error
    ...
    jmp 0x0000:0x1000 ; salto para onde o kernel foi carregado, agora o controle foi pro kernel

error:
    mov si, msg_error ; esse bloco todo vai ser uma rotina de erro
    call print_string
...
;Função pra imprimir as strings
print_string:
    mov ah, 0x0E      ; modo de saída de texto da BIOS
.print_loop:
    lodsb             ; lê um byte da string apontada por SI
    test al, al       ; se AL for zero chegou no fim da string e saí do loop
    jz .done
...
;Função que espera a tecla, retorna quando é pressionada
wait_key:
    mov ah, 0x00
    int 0x16
    ret               ; retorna o endereço de quem chamou o código que a chamou

;Mensagem de texto que vai aparecer 
